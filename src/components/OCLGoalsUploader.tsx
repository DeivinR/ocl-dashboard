    ...
    content: (
      <>
        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-ocl-primary/10 transition group-hover:bg-ocl-primary/20">
          <TrendingUp size={24} className="text-ocl-primary" />
        </div>
        <div className="text-center">
          <p className="text-sm font-semibold text-slate-700">Clique para selecionar o arquivo de metas OCL</p>
...
export const OCLGoalsUploader = () => {
  ...
  const handleFile = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setStatusMsg('Processando metas OCL...');

    try {
      await new Promise(resolve => setTimeout(resolve, 1500)); // Simulação

      setStatus('success');
      setStatusMsg('Metas OCL processadas com sucesso.');
      ...
      setStatus('idle');
      alert('Erro ao processar arquivo de metas OCL');
    }
  };
    ...
    <div className="mx-auto flex h-full max-w-xl flex-col items-center justify-center gap-4">
      <div className="w-full overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-5 py-3.5">
          <TrendingUp size={15} className="text-ocl-primary" />
          <span className="text-sm font-semibold text-slate-700">Upload de Metas OCL</span>
        </div>
